...
public class CowServiceGrpcImpl extends CowServiceGrpc.CowServiceImplBase {
    ...
    public void createCow(CreateCowRequest req, StreamObserver<com.wom.proto.Cow> resp) {
        try {
            var savedDto = service.createCow(req.getRegNo(), req.getBirthDateIso());
            var out = ProtoMappers.toProto(savedDto);
            resp.onNext(out);
            resp.onCompleted();
        } catch (Exception ex) {
            ex.printStackTrace(); // log do konsoli T3
            resp.onError(io.grpc.Status.INTERNAL
                .withDescription(ex.getClass().getSimpleName() + ": " + ex.getMessage())
                .asRuntimeException());
    }
}
}